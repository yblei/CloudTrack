    ...
    def __init__(
        ...
        *,
        ...
        if self.dataset == "vipseg":
            ...
            self.need_remapping = True
            self.visualize = True
        elif self.dataset == "flextrack":
            self.json_style = "flextrack"
            self.id2rgb_converter = ID2RGBConverter()
            self.need_remapping = True
            self.all_annotations = []
            dataset_name = path.dirname(video_name)
    ...
    def save_mask(
        ...
        args = ResultArgs(
            ...
            segments_info=copy.deepcopy(
                self.object_manager.get_current_segments_info()
            ),
        )
    ...
    while True:
        ...
        if saver.json_style == "vipseg":
            ...
            for seg in segments_info:
                ...
                coco_mask = mask_util.encode(
                    np.asfortranarray(seg["mask"].numpy())
                )
            ...
            saver.all_annotations.append(this_annotation)
        elif saver.json_style == "flextrack":
            ...
            for seg in segments_info:
                if best_seg is None or seg["score"] > best_seg["score"]:
                    best_seg = seg

            xyxy = [0, 0, 0, 0]
            if best_seg is not None:
                best_mask = mask == best_seg["id"] # only keep the best segment
                best_mask = torch.unsqueeze(best_mask, dim=0)
                if best_mask.sum() > 0: # we keep "None", if there is no mask for this detection in the frame. Weird, but it sometimes happens ...
                    xyxy = torchvision.ops.masks_to_boxes(best_mask).squeeze()
                    xyxy = xyxy.numpy()
                    xyxy = xyxy.tolist()
        ...
        # raise NotImplementedError("Diesen code muss ich mit meiner eigenen version überschreiben. Ich muss noch meinen eigenen json style definieren, in den ich wie unten die Boxen rein schreibe.")
        ...
        if save_the_mask:
            ...
            if saver.dataset != "gradio":
                ...
                if saver.output_postfix is not None:
                    this_out_path = path.join(
                        saver.output_root, saver.output_postfix
                    )
                else:
            ...
            if saver.visualize and saver.object_manager.use_long_id:
                if image_np is None:
                    if path_to_image is not None:
                        ...
                        raise ValueError(
                            "Cannot visualize without image_np or path_to_image"
                        )
                alpha = (out_mask == 0).astype(np.float32) * 0.5 + 0.5
                alpha = alpha[:, :, None]
                blend = (image_np * alpha + rgb_mask * (1 - alpha)).astype(
                    np.uint8
                )
                ...
                if prompts is not None:
                    ...
                    for seg in segments_info:
                        ...
                        labels.append(
                            f'{prompts[seg["category_id"]]} {seg["score"]:.2f}'
                        )
                    ...
                    if len(all_masks) > 0:
                        ...
                        blend = box_annotator.annotate(
                            scene=blend, detections=detections
                        )
                ...
                if saver.dataset != "gradio":
                    ...
                    if saver.visualize_postfix is not None:
                        this_out_path = path.join(
                            saver.output_root, saver.visualize_postfix
                        )
                    else:
                        this_out_path = saver.output_root
                    if saver.video_name is not None:
                        this_out_path = path.join(
                            this_out_path, saver.video_name
                        )

                    os.makedirs(this_out_path, exist_ok=True)
                    Image.fromarray(blend).save(
                        path.join(this_out_path, frame_name[:-4] + ".jpg")
                    )
                else:
